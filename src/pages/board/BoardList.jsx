...
export default function BoardList() {
  ...
  const canWrite = user && (category !== 'notice' || user.user_metadata?.is_admin)
  ...
  return (
    <section className="min-h-[calc(100vh-4rem)] section-x py-16 max-w-4xl mx-auto">
      ...
      {/* Category tabs */}
      <div className="flex border-b border-neutral-900 dark:border-okrr-cloud mb-0">
        {CATEGORIES.map((c) => (
          <button
            key={c.key}
            onClick={() => navigate(`/board/${c.key}`)}
          ...
          posts.map((post) => {
            ...
            return (
              ...
              >
                ...
                >
                  {category === 'notice' && (
                    <span className="mr-2 text-xs font-bold bg-neutral-900 dark:bg-okrr-cloud text-okrr-cloud dark:text-neutral-900 px-1.5 py-0.5 rounded">공지</span>
                  )}
                  {post.title}
                  {commentCount > 0 && (
      ...
      {!user && (
        <p className="mt-8 text-center text-sm text-neutral-400 dark:text-dark-muted">
          글을 작성하려면{' '}
          <Link to="/login" state={{ from: `/board/${category}` }} className="underline hover:text-neutral-900 dark:hover:text-okrr-cloud">
            로그인
          </Link>
          이 필요합니다.
        </p>
